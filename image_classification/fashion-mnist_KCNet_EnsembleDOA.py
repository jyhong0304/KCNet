...
parser = argparse.ArgumentParser(description='KCNet with Ensemble DOA for MNIST')
...
dataset = torchvision.datasets.FashionMNIST(root='~/data/fashion_mnist/', train=True, transform=transform,
                                            download=True)
test_dataset = torchvision.datasets.FashionMNIST(root='~/data/fashion_mnist/', train=False, transform=transform,
                                                 download=True)
...
for idx_submodel in range(opt.n_submodels):
    model = KCNet(input_size=image_size, h_size=opt.hsize, num_classes=num_classes, gen_S=True, device=device, reg=5)
    prev_acc = -1
...
model = KCNet(input_size=image_size, h_size=opt.hsize * opt.n_submodels, num_classes=num_classes,
              init_W=torch.cat(results_W, dim=0), device=device, reg=5)
model.fit(train_val_images, train_val_labels)
test_acc = model.evaluate(test_images, test_labels)
